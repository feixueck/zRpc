...
//    public static final String L_NODE = "/zs";
    private static final String L_NODE = "/example/cache";
    ...
    public static void main(String[] args) throws Exception {
        CuratorFramework client = CuratorFrameworkFactory.newClient(ZConstants.CONNECTION_INFO, new ExponentialBackoffRetry(1000, 3));
        client.start();
...
//                    Thread.sleep(3000L);
//                    //
//                    Stat stat = client.checkExists().forPath(L_NODE);
//                    try {
//                        client.setData().forPath(L_NODE);
//                    } catch (Exception e) {
//                        client.create().forPath(L_NODE, "please".getBytes());
//                    }
////                    client.create().creatingParentsIfNeeded().forPath("/test");
                    String small = ZKPaths.makePath(L_NODE, "small");
//                    //update
//                    try {
//                        client.setData().forPath(small, "small".getBytes());
//                    } catch (Exception e) {
//                        client.create().creatingParentsIfNeeded().forPath(small, "no node,so create".getBytes());
//                    }
//                    //delte
//                    Thread.sleep(1000L);
//                    client.delete().guaranteed().deletingChildrenIfNeeded().forPath(small);
                    //create
                    for (int i = 0; i < 10; i++) {
                        Thread.sleep(1000);
                        Stat stat1 = client.checkExists().forPath(small);
                        System.out.println(stat1);
                        if (stat1 == null) {
                            client.create().creatingParentsIfNeeded().forPath(small, (i + "xx").getBytes());
                        } else {
                            client.delete().guaranteed().deletingChildrenIfNeeded().forPath(small);
                        }
                        //
    ...
    private static CuratorFramework clientOne() {
        //zk 地址
        String connectString = ZConstants.CONNECTION_INFO;
        // 连接时间 和重试次数
...
//        ExecutorService pool = Executors.newCachedThreadPool();
        PathChildrenCache childrenCache = new PathChildrenCache(client, L_NODE, true);
