...
public class PathCacheExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            client = CuratorFrameworkFactory.newClient(ZConstants.CONNECTION_INFO, new ExponentialBackoffRetry(1000, 3));
            client.start();
...
//            processCommands(client, cache);
            ...
            for (int i = 0; i < 10; i++) {
                Thread.sleep(1000);
                Stat stat1 = client.checkExists().forPath(PATH + "/" + i);
                System.out.println(stat1);
                try {
                    client.setData().forPath(PATH + "/" + i, (i + "xx").getBytes());
                } catch (KeeperException.NoNodeException e) {
                    client.create().creatingParentsIfNeeded().forPath(PATH + "/" + i, (i + "xxss").getBytes());
                }
//                if (stat1 == null) {
//                    client.create().creatingParentsIfNeeded().forPath(PATH + "/" + i, (i + "xx").getBytes());
//                } else {
//                    client.delete().guaranteed().deletingChildrenIfNeeded().forPath(PATH + "/" + i);
//                }
                //
            }
            System.in.read();
